...
class LoadImage:
    ...
    @classmethod
    def save_input_from_gradio(self, image):
        input_folder_path = folder_paths.get_input_directory()
        import time
        filename = "input_" + str(int(time.time())) +".png"
        input_file_store_at = os.path.join(input_folder_path, filename)
        print(input_file_store_at)
        image.save(input_file_store_at)
        return input_file_store_at
